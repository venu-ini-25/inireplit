...
router.get("/portfolio/summary", async (_req, res) => {
  ...
  const totalValuation = list.reduce((s, c) => s + c.valuation, 0);
  const avgGrowthRate = list.length
    ? parseFloat((list.reduce((s, c) => s + c.growthRate, 0) / list.length).toFixed(1))
    : 53.0;
  const avgValuation = list.length ? totalValuation / list.length : 0;
  const totalAum = totalValuation > 0 ? totalValuation : 688500000;
