...
export default function SignUp() {
  ...
  const handleSSO = async (provider: "Google" | "Microsoft" | "Apple") => {
    ...
    setLoading(null);
    setSsoMsg(`${provider} SSO is available for enterprise plans. Create an account with email below to access the demo, or contact us to enable SSO for your organization.`);
  };

  const validate = () => {
    if (!form.firstName.trim()) return "First name is required.";
    if (!form.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) return "A valid email is required.";
    if (!passwordRules.every((r) => r.test(form.password))) return "Password does not meet requirements.";
    return null;
  };
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground">Join iNi and request access to the platform demo</p>
          </div>
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                  Company / Fund <span className="text-muted-foreground font-normal">(optional)</span>
                </label>
                <input
                  value={form.company}
                  ...
                  <div className="mt-2 space-y-1">
                    {passwordRules.map((rule) => (
                      <div key={rule.label} className="flex items-center gap-1.5">
                        <CheckCircle2 className={`w-3.5 h-3.5 ${rule.test(form.password) ? "text-success" : "text-slate-300"}`} />
                        <span className={`text-[11px] ${rule.test(form.password) ? "text-success" : "text-muted-foreground"}`}>{rule.label}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <p className="text-center text-xs text-muted-foreground mt-5 leading-relaxed">
            By creating an account, you agree to iNi's{" "}
