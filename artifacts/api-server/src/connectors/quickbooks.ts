import { db, integrationConnections, financialSnapshots, syncLogs } from "@workspace/db";
...
async function refreshTokens(conn: IntegrationConnection): Promise<IntegrationConnection> {
  if (!QB_CLIENT_ID || !QB_CLIENT_SECRET || !conn.refreshToken) throw new Error("Cannot refresh: missing credentials or refresh token");
  const creds = Buffer.from(`${QB_CLIENT_ID}:${QB_CLIENT_SECRET}`).toString("base64");
  const resp = await fetch(QB_TOKEN_URL, {
    method: "POST",
    headers: { "Authorization": `Basic ${creds}`, "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json" },
...
async function getAccessToken(conn: IntegrationConnection): Promise<string> {
...
export async function sync(connectionId: string): Promise<{ recordsSynced: number }> {
  ...
  try {
    ...
    const plResp = await fetch(
      `${QB_API_BASE}/${realmId}/reports/ProfitAndLoss?start_date=${startDate}&end_date=${endDate}&summarize_column_by=Month`,
      { headers: { "Authorization": `Bearer ${token}`, "Accept": "application/json" } }
    );
    if (!plResp.ok) throw new Error(`QB P&L API error: ${await plResp.text()}`);
    const plData = await plResp.json() as Record<string, unknown>;

    const columns = (plData["Columns"] as Record<string, unknown>)?.["Column"] as Record<string, unknown>[] ?? [];
    const rows = ((plData["Rows"] as Record<string, unknown>)?.["Row"] as Record<string, unknown>[]) ?? [];

    const monthLabels = columns.slice(1, -1).map((c) => String((c["ColTitle"] as string) || ""));

    const findSection = (name: string) => rows.find((r) => {
      const header = (r["Header"] as Record<string, unknown>)?.["ColData"] as Record<string, unknown>[];
      return header?.[0]?.["value"] === name;
    });

    const getMonthValues = (section: Record<string, unknown> | undefined): number[] => {
      if (!section) return monthLabels.map(() => 0);
      const summary = (section["Summary"] as Record<string, unknown>)?.["ColData"] as Record<string, unknown>[];
      if (!summary) return monthLabels.map(() => 0);
      return summary.slice(1, -1).map((c) => Number(String(c["value"] || "0").replace(/,/g, "")) || 0);
    };

    const revenues = getMonthValues(findSection("Income"));
    const expenses = getMonthValues(findSection("Expenses"));
