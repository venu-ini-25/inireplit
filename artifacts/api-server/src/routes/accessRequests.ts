...
import { notifyAdminNewRequest, notifyUserApproved, notifyUserDenied } from "../lib/email";
...
router.post("/access-requests", async (req, res) => {
  ...
  notifyAdminNewRequest({ id, firstName: firstName ?? "", lastName: lastName ?? "", email: email ?? "", company: company ?? "", role: role ?? "", aum: aum ?? "", message: message ?? "" }).catch(() => {});
...
router.get("/admin", async (req, res) => {
  ...
  if (action === "approve") {
    ...
    if (rows.length === 0) return res.status(404).json({ error: "Request not found" });
    const req = rowToRequest(rows[0]);
    notifyUserApproved({ id: req.id, firstName: req.firstName, lastName: req.lastName, email: req.email, company: req.company, role: req.role, aum: req.aum }, safePlatform).catch(() => {});
    return res.json(req);
  }
  ...
  if (action === "deny") {
    ...
    if (rows.length === 0) return res.status(404).json({ error: "Request not found" });
    const req = rowToRequest(rows[0]);
    notifyUserDenied({ id: req.id, firstName: req.firstName, lastName: req.lastName, email: req.email, company: req.company, role: req.role, aum: req.aum }).catch(() => {});
    return res.json(req);
  }
...
async function approveRequest(id: string, platformAccess = "demo") {
  const { rows } = await pool.query(
    "UPDATE access_requests SET status = 'approved', platform_access = $1, reviewed_at = NOW() WHERE id = $2 RETURNING *",
    [platformAccess, id]
  );
  if (rows.length > 0) {
    const req = rowToRequest(rows[0]);
    notifyUserApproved({ id: req.id, firstName: req.firstName, lastName: req.lastName, email: req.email, company: req.company, role: req.role, aum: req.aum }, platformAccess).catch(() => {});
  }
  return rows;
}
