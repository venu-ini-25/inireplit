    ...
    <aside className="w-64 h-screen flex flex-col bg-white border-r border-border z-40">
      <div className="h-20 md:h-24 flex items-center px-4 border-b border-border shrink-0 gap-2">
        ...
        {isAdminOnly ? (
          ...
          <button
            onClick={toggleMode}
            title={platformAccess === "demo" ? "Switch to Live data (real database)" : "Switch to Demo mode (sample data)"}
            className={cn(
      ...
      <div className="flex-1 py-4 px-3 flex flex-col gap-0.5 overflow-y-auto">
        ...
        {!isAdminOnly && (
          <>
            ...
            <Link href="/data-agent" className={linkCls("/data-agent")} onClick={navClick}>
              <Database className="w-4 h-4 shrink-0" />
              Data Agent
              <span className="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-primary/10 text-primary uppercase tracking-wide">AI</span>
            </Link>
          </>
        )}
